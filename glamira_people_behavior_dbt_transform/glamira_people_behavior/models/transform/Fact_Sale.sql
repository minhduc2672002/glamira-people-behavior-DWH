...
fact_sale__group_value AS (
    SELECT
        ...
        currency,
        MAX(CASE 
                WHEN option.option_label = 'alloy' THEN option.value_label
                ELSE NULL
            END) AS alloy_value,
        MAX(CASE 
                WHEN option.option_label = 'diamond' THEN option.value_label
                ELSE NULL
...
fact_sale__format_value AS (
    SELECT
        FORMAT_TIMESTAMP('%Y%m%d', TIMESTAMP_SECONDS(time_stamp)) AS full_date,
        ip,
        product_id,
        diamond_value,
        {{ normalize_price('price') }} AS price, -- Chuẩn hóa `price`
        amount,
        currency,
        {{ normalize_alloy_value('alloy_value') }} AS alloy_value -- Chuẩn hóa `alloy_value`
